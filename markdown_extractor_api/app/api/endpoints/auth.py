...
async def create_api_key(user_id: uuid.UUID, key_in: ApiKeyCreate, db: AsyncSession = Depends(get_db)):
    ...
    keys_query = select(ApiKey).where(ApiKey.user_id == user.id)
    keys_result = await db.execute(keys_query)
    existing_keys = keys_result.scalars().all()

    if len(existing_keys) >= 2:
        raise HTTPException(status_code=403, detail="Free tier is limited to 2 active project keys. Please upgrade.")
    ...
    return {
        ...
        "prefix": key_prefix,
        "warning": "Please copy this key immediately."
    }
    ...
    keys_data = [
        {
            ...
            "key": f"{k.prefix}****************", 
            "prefix": k.prefix,
            "requests_used": getattr(k, 'requests_used', 0),
            "limit": getattr(k, 'request_limit', 10),
            "created_at": k.created_at.strftime("%Y-%m-%d") if getattr(k, 'created_at', None) else "Today"
        } 
        for k in api_keys
    ]
...
async def delete_api_key(prefix: str, db: AsyncSession = Depends(get_db)):
